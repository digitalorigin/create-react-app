...
const RollbarSourceMapPlugin = require('rollbar-sourcemap-webpack-plugin'); // PG
const BundleAnalyzerPlugin = require('webpack-bundle-analyzer')
  .BundleAnalyzerPlugin; // PG
...
module.exports = function(webpackEnv) {
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    ...
    if (preProcessor) {
      ...
      loaders.push(isEnvDevelopment ? pgLoaderDev : pgLoaderProd); // PG
      loaders.push(...extraSassLoaders); // PG
    }
    return loaders;
  };

  // Start adding pagantis custom webpack plugins
  const doPlugins = isEnvProduction
    ? [new webpack.optimize.ModuleConcatenationPlugin()]
    : []; // PG

  // Add rollbar only if react app env is production and we have rollbar token
  if (process.env.ROLLBAR_SERVER_TOKEN && isEnvProduction) {
    doPlugins.push(
  ...
  if (process.env.ANALYZER && isEnvProduction) {
    doPlugins.push(new BundleAnalyzerPlugin({ defaultSizes: 'gzip' }));
  }

  // end adding pagantis custom webpack plugins

  return {
    mode: isEnvProduction ? 'production' : isEnvDevelopment && 'development',
    // Stop compilation early in production
    bail: isEnvProduction,
    devtool: isEnvProduction ? 'hidden-source-map' : 'cheap-module-source-map', // PG
      ...
      rules: [
        ...
        },
        {
          ...
          oneOf: [
            ...
            {
              test: /\.(js|mjs|jsx|ts|tsx)$/,
              include: [paths.appSrc, paths.doComponentModulesRegex], // PG
              loader: require.resolve('babel-loader'),
              options: {
                customize: require.resolve(
                  '@digital-origin/babel-preset-react-app/webpack-overrides' // PG
                ),
                // @remove-on-eject-begin
                babelrc: false,
                configFile: false,
                ...
                cacheIdentifier: getCacheIdentifier(
                  ...
                  [
                    ...
                    '@digital-origin/babel-preset-react-app', // PG
                    'react-dev-utils',
                    '@digital-origin/react-scripts', // PG
                  ]
                ),
                // @remove-on-eject-end
                plugins: [
            ...
            // Unlike the application JS, we only compile the standard ES features.
            {
              ...
              options: {
                ...
                presets: [
                  [
                    require.resolve(
                      '@digital-origin/babel-preset-react-app/dependencies'
                    ), // PG
                    { helpers: true },
                  ],
                ],
      ...
      new HtmlWebpackPlugin(
        Object.assign(
          {},
          {
            ...
            insertRollbar: false, // PG
            insertNewrelic: false, // PG
          },
          isEnvProduction
            ? {
                minify: {
